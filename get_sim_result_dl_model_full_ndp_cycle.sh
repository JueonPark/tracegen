...
CSV_FW_PATH=$CSV_PATH-fw-ndp-full-cycle.csv
CSV_BW_PATH=$CSV_PATH-bw-ndp-full-cycle.csv
...
do
    ...
    NDP_STRING=`cat sim_result.out | grep "at cycle" | grep "NDP" | tail -n1 `
    NDP_SPLIT=(${NDP_STRING//' '/ })
    NDP_CYCLE=${NDP_SPLIT[10]}
    ...
    if [ $CYCLE_CNT -ne 2 ]; then  
        ...
        echo $DEVICE_SETTING,$CONFIG,$5,$line,forward,NDP_OP,$NDP_CYCLE >> $CSV_FW_PATH ;
    fi
    popd
done
...
do
    ...
    if [ $CYCLE_CNT -ne 2 ]; then  
        ...
        echo $DEVICE_SETTING,$CONFIG,$5,$line,backward,NDP_OP,$NDP_CYCLE >> $CSV_BW_PATH ;
    fi
    popd
done
