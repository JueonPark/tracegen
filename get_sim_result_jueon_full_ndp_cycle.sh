...
CSV_PATH=$CSV_PATH-full-ndp-cycle.csv
...
do
  ...
  CYCLE=`cat sim_result.out | grep "NDP kernel" | grep "launched" | tail -n1 | awk '{print($11)}'` 
  CYCLE2=`cat sim_result.out | grep "NDP kernel" | grep "finished" | tail -n1 | awk '{print($11)}'` 
  GPU_CYCLE=`cat GPU_0.out | grep "sim_cycle" | tail -n1 | awk '{print($3)}'` 
  ...
  if [ $CYCLE_CNT -ne 2 ]; then  
      ...
      echo $DEVICE_SETTING,$CONFIG,$5,$line,$NAME,$GPU_CYCLE >> $CSV_PATH ;
